...
def build_status_matrix(rows: list[dict[str, str]]) -> tuple[list[float], list[float], list[list[int]]]:
    omega_reference_ratios = sorted(
        {float(row["omega_reference_ratio"]) for row in rows}
    )
    omega_cd_over_omega = sorted(
        {float(row["omega_cd_over_omega"]) for row in rows}
    )

    matrix = [
        [None for _ in omega_reference_ratios] for _ in omega_cd_over_omega
    ]

    x_index = {value: index for index, value in enumerate(omega_reference_ratios)}
    y_index = {value: index for index, value in enumerate(omega_cd_over_omega)}
    ...
    for row in rows:
        ...
        y_value = float(row["omega_cd_over_omega"])
        if y_value > 0.0 and status != "ambiguous":
            raise ValueError(
                "nonzero omega_CD rows must remain ambiguous under the current "
                f"contract, got {status} for omega_CD/omega={y_value}"
            )
        ...
        if matrix[y_position][x_position] is not None:
            raise ValueError(
                ...
                f"omega/omega_0={x_value}, omega_CD/omega={y_value}"
            )
        ...
        for x_position, value in enumerate(row):
            if value is None:
                raise ValueError(
                    ...
                    f"omega_CD/omega={omega_cd_over_omega[y_position]}"
                )

    return omega_reference_ratios, omega_cd_over_omega, matrix


def render_figure(
    omega_reference_ratios: list[float],
    omega_cd_over_omega: list[float],
    ...
    ax.set_yticks(range(len(omega_cd_over_omega)))
    ax.set_yticklabels([f"{value:g}" for value in omega_cd_over_omega])
    ax.set_xlabel(r"$\omega / \omega_0$")
    ax.set_ylabel(r"$\omega_{CD} / \omega$")
    ax.set_title("CD-Rotation Perturbation Status Matrix")
    ...
    ax.set_yticks([tick - 0.5 for tick in range(1, len(omega_cd_over_omega))], minor=True)
    ...
    fig.text(
        0.5,
        0.035,
        "Control row retains the special-case transition. Every nonzero "
        r"$\omega_{CD}$ row remains ambiguous because no family-specific gliding "
        r"or $\tau_{contact}$ law is available in the current source set.",
        ha="center",
        va="center",
        fontsize=10,
    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Render the canonical CD-rotation perturbation figure."
    )
    parser.add_argument(
        "--input",
        default=repo_root / "results" / "perturbation_cd_rotation.csv",
        type=Path,
        help="Path to the canonical CD-rotation perturbation CSV.",
    )
    parser.add_argument(
        "--output",
        default=repo_root / "figures" / "perturbation_cd_rotation.png",
        type=Path,
    ...
    omega_reference_ratios, omega_cd_over_omega, matrix = build_status_matrix(rows)
    render_figure(omega_reference_ratios, omega_cd_over_omega, matrix, args.output)
