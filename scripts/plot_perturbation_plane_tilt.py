...
def build_status_matrix(rows: list[dict[str, str]]) -> tuple[list[float], list[float], list[list[int]]]:
    ...
    plane_tilt_degrees = sorted({float(row["plane_tilt_degrees"]) for row in rows})

    matrix = [[None for _ in omega_reference_ratios] for _ in plane_tilt_degrees]

    x_index = {value: index for index, value in enumerate(omega_reference_ratios)}
    y_index = {value: index for index, value in enumerate(plane_tilt_degrees)}
    ...
    for row in rows:
        ...
        y_value = float(row["plane_tilt_degrees"])
        if y_value > 0.0 and status != "ambiguous":
            raise ValueError(
                "nonzero plane_tilt rows must remain ambiguous under the current "
                f"contract, got {status} for plane_tilt_degrees={y_value}"
            )
        ...
        if matrix[y_position][x_position] is not None:
            raise ValueError(
                ...
                f"omega/omega_0={x_value}, plane_tilt_degrees={y_value}"
            )
        ...
        for x_position, value in enumerate(row):
            if value is None:
                raise ValueError(
                    ...
                    f"plane_tilt_degrees={plane_tilt_degrees[y_position]}"
                )

    return omega_reference_ratios, plane_tilt_degrees, matrix


def render_figure(
    omega_reference_ratios: list[float],
    plane_tilt_degrees: list[float],
    ...
    ax.set_yticks(range(len(plane_tilt_degrees)))
    ax.set_yticklabels([f"{value:g}" for value in plane_tilt_degrees])
    ax.set_xlabel(r"$\omega / \omega_0$")
    ax.set_ylabel("plane tilt (degrees)")
    ax.set_title("Plane-Tilt Perturbation Status Matrix")
    ...
    ax.set_yticks([tick - 0.5 for tick in range(1, len(plane_tilt_degrees))], minor=True)
    ...
    fig.text(
        0.5,
        0.035,
        "Zero-tilt control retains the special-case transition. Every nonzero "
        r"plane-tilt row remains ambiguous because no family-specific gliding "
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Render the canonical plane-tilt perturbation figure."
    )
    parser.add_argument(
        "--input",
        default=repo_root / "results" / "perturbation_plane_tilt.csv",
        type=Path,
        help="Path to the canonical plane-tilt perturbation CSV.",
    )
    parser.add_argument(
        "--output",
        default=repo_root / "figures" / "perturbation_plane_tilt.png",
        type=Path,
    ...
    omega_reference_ratios, plane_tilt_degrees, matrix = build_status_matrix(rows)
    render_figure(omega_reference_ratios, plane_tilt_degrees, matrix, args.output)
